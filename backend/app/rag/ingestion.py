...
def _chunk_and_store_transcript(INDEX, video_id: str, source: str, metadata: Dict, transcript: str, ids: list, metadatas: list, chunk_size: Optional[int] = None, overlap: Optional[int] = None):
    ...
    if not INDEX:
        ...
        return {"chunks_stored": len(records), "metadata_stored": False}

    try:
        INDEX.upsert_records(namespace="videometric", records=records)
...
def chunk_and_store(INDEX, ids: list,metadatas: list,payload: TranscriptResponse | Dict[str, Any]):
